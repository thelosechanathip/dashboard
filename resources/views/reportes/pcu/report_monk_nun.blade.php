...
@section('title')
    <title>รายงานรายชื่อพระและแม่ชี</title>
@endsection
...
@section('content')
    <main class="main-content mb-5">
        {{-- Title แสดงข้อมูล ชื่อผู้ใช้งาน และ แผนก Start --}}
            <div class="card shadow-lg full-width-bar p-3" data-aos="fade-left" data-aos-easing="linear" data-aos-duration="400">
                <div
                    ...
                    <div class="d-flex">
                        <h1 class="h2">รายงานรายชื่อพระและแม่ชี</h1>
                    </div>
                </div>
            </div>
        ...
        {{-- All Form Date End --}}
        {{-- Show Table Start --}}
            <div class="mt-3 card shadow-lg w-100" id="report_monk_nun_table">
                <div class="mx-5 w-auto">
                    <div class="my-5 ms-0 w-auto">
                        <div class="w-auto" id="fetch-report-monk-nun"></div>
                    </div>
                </div>
            </div>
        {{-- Show Table End --}}
...
@section('script')
    <script>
        $(document).ready(function() {
            ...
            // Set Text Start
                function setText(request) {
                    if(request == 0) {
                        ...
                        $('#setCount').show();
                        $('#setText').text('');
                        $('#setCount').text('ไม่มีรายการพระและแม่ชี');
                    } else {
                        $('#setText').show();
                        $('#setCount').show();
                        $('#setText').text('จำนวนรายการพระและแม่ชี : ');
            ...
            // ดึงข้อมูลรายชื่อพระและแม่ชี ตามปีงบประมาณ ที่เลือกเอง Start
                $('#submitYear').click(function(e) {
                    ...
                    $.ajax({
                        url: '{{ route('getReportMonkNunFetchYear') }}',
                        type: 'GET',
                        data: formData,
                        success: function(response) {
                            ...
                            if(response.status === 400) {
                                ...
                                $('#report_monk_nun_table').show();
                                $('#setText').hide();
                                $('#setCount').hide();
                                $("#fetch-report-monk-nun").html(response);
                                $("#table-fetch-report-monk-nun").DataTable({
                                    responsive: true,
                                    order: [0, 'desc'],
                                    autoWidth: false,
                                    buttons: ['excel'],
            ...
            // ดึงข้อมูลรายชื่อพระและแม่ชี ตามปีงบประมาณ ที่เลือกเอง End

            // ดึงข้อมูลรายชื่อพระและแม่ชี ตามวัน-เดือน-ปี ที่เลือกเอง Start
                $('#submitAll').click(function(e) {
                    ...
                    $.ajax({
                        url: '{{ route('getReportMonkNunFetchSelectDate') }}',
                        type: 'GET',
                        data: formData,
                        success: function(response) {
                            Swal.close();
            ...
            // ดึงข้อมูลรายชื่อพระและแม่ชี ตามวัน-เดือน-ปี ที่เลือกเอง End
