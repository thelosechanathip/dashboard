...
@section('title')
    <title>รายชื่อผู้ป่วยอายุ 45-70 ปี ที่ตรวจ Cholesterol โดยไม่รวม ICD10 (E70-E79, Z136) และไม่ใช้ Simvastatin, Atorvastatin</title>
@endsection

@section('content')
    <main class="main-content mb-5" id="choresterol_main">
        {{-- Title แสดงข้อมูล ชื่อผู้ใช้งาน และ แผนก Start --}}
            <div class="card shadow-lg full-width-bar p-3" data-aos="fade-left" data-aos-easing="linear" data-aos-duration="400">
                ...
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center mt-3">
                    <div class="d-flex">
                        <h1 class="h2">รายชื่อผู้ป่วยอายุ 45-70 ปี ที่ตรวจ Cholesterol โดยไม่รวม ICD10 (E70-E79, Z136) และไม่ใช้ Simvastatin, Atorvastatin</h1>
                    </div>
                </div>
            </div>
        ...
        {{-- All Form Date End --}}
        {{-- Show Table Start --}}
            <div class="mt-3 card shadow-lg w-100" id="report_choresterol_table">
                <div class="mx-5 w-auto">
                    <div class="my-5 ms-0 w-auto">
                        <div class="w-auto" id="fetch-report-choresterol"></div>
                    </div>
                </div>
            </div>
        {{-- Show Table End --}}
        ...
        $(document).ready(function() {
            ...
            // เริ่มต้น Hide Start
                ...
                function hideAll() {
                    ...
                    $('#report_choresterol_table').hide(); // ปิดการมองเห็นของ Card ที่เก็บ Table ที่ดึงข้อมูลมาแสดงบนหน้า HTML
            ...
            // ดึงข้อมูลรายชื่อที่มี ICD 10 ตามปีงบประมาณ ที่เลือกเอง Start
                $('#submitYear').click(function(e) {
                    ...
                    $.ajax({
                        url: '{{ route('getReportChoresterolFetch') }}',
                        type: 'GET',
                        data: formData,
                        success: function(response) {
                            ...
                            if(response.status === 400) {
                                ...
                                $('#report_choresterol_table').show();
                                $("#fetch-report-choresterol").html(response);
                                $("#table-fetch-report-choresterol").DataTable({
                                    responsive: true,
                                    order: [0, 'desc'],
                                    autoWidth: false,
                                    fixedHeader: true,
