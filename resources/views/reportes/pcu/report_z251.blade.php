...
@section('content')
    <main class="main-content mb-5">
        ...
        {{-- All Form Date End --}}
        {{-- Show Table Start --}}
            <div class="mt-3 card shadow-lg w-100" id="report_z251_table">
                <div class="mx-5 w-auto">
                    <div class="my-5 ms-0 w-auto">
                        <div class="w-auto" id="fetch-report-z251"></div>
                    </div>
                </div>
            </div>
        {{-- Show Table End --}}
        ...
        $(document).ready(function() {
            ...
            // Hide Form && Chart && Table Start
                function hideForm() {
                    ...
                    $('#setCount').hide();
                    $('#report_z251_table').hide();
                    $('#report_z251_form').hide();
                    $('#select').val('0');
                }
            // Hide Form && Chart && Table End

            // ดึงข้อมูลรายชื่อที่มี ICD Z251 ตามปีงบประมาณ ที่เลือกเอง Start
                $('#submitYear').click(function(e) {
                    ...
                    $.ajax({
                        url: '{{ route('getReportZ251FetchYear') }}',
                        type: 'GET',
                        data: formData,
                        success: function(response) {
                            ...
                            if(response.status === 400) {
                                ...
                                $('#report_z251_table').show();
                                $('#setText').hide();
                                $('#setCount').hide();
                                $("#fetch-report-z251").html(response);
                                $("#table-fetch-report-z251").DataTable({
                                    responsive: true,
                                    order: [0, 'desc'],
                                    autoWidth: false,
                                    buttons: ['excel'],
            ...
            // ดึงข้อมูลรายชื่อที่มี ICD Z251 ตามปีงบประมาณ ที่เลือกเอง End

            // ดึงข้อมูลรายชื่อที่มี ICD Z251 ตามวัน-เดือน-ปี ที่เลือกเอง Start
                $('#submitAll').click(function(e) {
                    ...
                    $.ajax({
                        url: '{{ route('getReportZ251FetchAll') }}',
                        type: 'GET',
                        data: formData,
                        success: function(response) {
                            Swal.close();
            ...
            // ดึงข้อมูลรายชื่อที่มี ICD Z251 ตามวัน-เดือน-ปี ที่เลือกเอง End
