...
@section('content')
    <main class="main-content mb-5">
        ...
        {{-- All Form Date End --}}
        {{-- Show Table Start --}}
            <div class="mt-3 card shadow-lg w-100" id="report_z242_table">
                <div class="mx-5 w-auto">
                    <div class="my-5 ms-0 w-auto">
                        <div class="w-auto" id="fetch-report-z242"></div>
                    </div>
                </div>
            </div>
        {{-- Show Table End --}}
        ...
        $(document).ready(function() {
            ...
            // Set Text Start
                function setText(request) {
                    if(request == 0) {
                        ...
                        $('#setCount').show();
                        $('#setText').text('');
                        $('#setCount').text('ไม่มีรายการ ICD Z242');
                    } else {
                        $('#setText').show();
                        $('#setCount').show();
                        $('#setText').text('จำนวนรายการ ICD Z242 : ');
            ...
            // Hide Form && Chart && Table Start
                function hideForm() {
                    ...
                    $('#setCount').hide();
                    $('#report_z242_table').hide();
                    $('#report_z242_form').hide();
                    $('#select').val('0');
                }
            // Hide Form && Chart && Table End

            // ดึงข้อมูลรายชื่อที่มี ICD Z242 ตามปีงบประมาณ ที่เลือกเอง Start
                $('#submitYear').click(function(e) {
                    ...
                    $.ajax({
                        url: '{{ route('getReportZ242FetchYear') }}',
                        type: 'GET',
                        data: formData,
                        success: function(response) {
                            ...
                            if(response.status === 400) {
                                ...
                                $('#report_z242_table').show();
                                $('#setText').hide();
                                $('#setCount').hide();
                                $("#fetch-report-z242").html(response);
                                $("#table-fetch-report-z242").DataTable({
                                    responsive: true,
                                    order: [0, 'desc'],
                                    autoWidth: false,
                                    buttons: ['excel'],
            ...
            // ดึงข้อมูลรายชื่อที่มี ICD Z242 ตามปีงบประมาณ ที่เลือกเอง End

            // ดึงข้อมูลรายชื่อที่มี ICD Z242 ตามวัน-เดือน-ปี ที่เลือกเอง Start
                $('#submitAll').click(function(e) {
                    ...
                    $.ajax({
                        url: '{{ route('getReportZ242FetchAll') }}',
                        type: 'GET',
                        data: formData,
                        success: function(response) {
                            Swal.close();
            ...
            // ดึงข้อมูลรายชื่อที่มี ICD Z242 ตามวัน-เดือน-ปี ที่เลือกเอง End
