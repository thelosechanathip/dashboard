...
@section('title')
    <title>รายงาน Admit ตึกรวม</title>
@endsection
...
@section('content')
    <main class="main-content mb-5">
        {{-- Title แสดงข้อมูล ชื่อผู้ใช้งาน และ แผนก Start --}}
            <div class="card shadow-lg full-width-bar p-3" data-aos="fade-left" data-aos-easing="linear" data-aos-duration="400">
                <div
                    ...
                    <div class="d-flex">
                        <h1 class="h2">รายงาน Admit ตึกรวม</h1>
                    </div>
                </div>
            </div>
        ...
        {{-- All Form Date End --}}
        {{-- Show Table Start --}}
            <div class="mt-3 card shadow-lg w-100" id="report_mixed_building_table">
                <div class="mx-5 w-auto">
                    <div class="my-5 ms-0 w-auto">
                        <div class="w-auto" id="fetch-report-mixed-building"></div>
                    </div>
                </div>
            </div>
        {{-- Show Table End --}}
...
@section('script')
    <script>
        $(document).ready(function() {
            ...
            // เริ่มต้น Hide Start
                $('#report_mixed_building_table').hide();
                $('#yearForm').hide();
                // $('#monthForm').hide();
                $('#allForm').hide();
                Swal.close();
            ...
            // Setting ตัวเลือก Form การใช้งาน Start
            ...
            // Setting ตัวเลือก Form การใช้งาน End
            ...
            // Set Text Start
                function setText(request) {
                    if(request == 0) {
                        ...
                        $('#setCount').show();
                        $('#setText').text('');
                        $('#setCount').text('ไม่มีรายการ Mixed Building');
                    } else {
                        $('#setText').show();
                        $('#setCount').show();
                        $('#setText').text('จำนวนรายการ Mixed Building : ');
            ...
            // Hide Form && Chart && Table Start
                function hideForm() {
                    ...
                    $('#setCount').hide();
                    $('#report_mixed_building_table').hide();
                    $('#report_mixed_building_form').hide();
                    $('#select').val('0');
                }
            // Hide Form && Chart && Table End

            // ดึงข้อมูลรายชื่อที่มี Mixed Building ตามปีงบประมาณ ที่เลือกเอง Start
                $('#submitYear').click(function(e) {
                    ...
                    $.ajax({
                        url: '{{ route('getReportMixedBuildingFetchYear') }}',
                        type: 'GET',
                        data: formData,
                        success: function(response) {
                            ...
                            if(response.status === 400) {
                                ...
                                $('#report_mixed_building_table').show();
                                $('#setText').hide();
                                $('#setCount').hide();
                                $("#fetch-report-mixed-building").html(response);
                                $("#table-fetch-report-mixed-building").DataTable({
                                    responsive: true,
                                    order: [0, 'desc'],
                                    autoWidth: false,
                                    buttons: ['excel'],
            ...
            // ดึงข้อมูลรายชื่อที่มี Mixed Building ตามปีงบประมาณ ที่เลือกเอง End

            // ดึงข้อมูลรายชื่อที่มี Mixed Building ตามวัน-เดือน-ปี ที่เลือกเอง Start
                $('#submitAll').click(function(e) {
                    ...
                    $.ajax({
                        url: '{{ route('getReportMixedBuildingFetchSelectDate') }}',
                        type: 'GET',
                        data: formData,
                        success: function(response) {
                            Swal.close();
            ...
            // ดึงข้อมูลรายชื่อที่มี Mixed Building ตามวัน-เดือน-ปี ที่เลือกเอง End
