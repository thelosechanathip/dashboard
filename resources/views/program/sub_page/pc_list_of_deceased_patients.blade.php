...
@section('title')
    <title>ทะเบียนคนไข้ที่เสียชีวิต</title>
@endsection

@section('content')
    <!-- Modal เยี่ยมบ้าน รพ.ครั้ง Start -->
        {{-- ทะเบียนเยี่ยมบ้าน รพ.(ครั้ง) Start --}}
            <div class="modal fade " id="list_of_deceased_patients_home_visiting_information" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-custom modal-dialog-centered mt-5">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="list_of_deceased_patients_home_visiting_information_title">ข้อมูลการเยี่ยมบ้าน</h5>
                            <button type="button" class="btn-close zoom-card" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="container" id="show-list-of-deceased-patients-home-visiting-information"></div>
                        </div>
    ...
    <main class="main-content mb-5" id="main">
        {{-- Title แสดงข้อมูล ชื่อผู้ใช้งาน และ แผนก Start --}}
            <div class="card shadow-lg full-width-bar p-3" data-aos="fade-left" data-aos-easing="linear" data-aos-duration="400">
                <div
                    ...
                    <div class="d-flex">
                        <h1 class="h2">ทะเบียนคนไข้เสียชีวิต</h1>
                    </div>
                </div>
            </div>
        ...
        {{-- All Form Date Start --}}
            <div class="mt-3 card shadow-lg full-width-bar p-3" id="form_all">
                <div class="d-flex justify-content-center align-items-center mt-4">
                    {{-- Form ดึงรายชื่อคนไข้ที่เสียชีวิต Start --}}
                        <form id="list_of_deceased_patients_list_name_form">
                            @csrf
                            <div class="mb-3 d-flex align-items-center row">
                                ...
                                <div class="col d-flex align-items-center">
                                    <button type="button" id="list_of_deceased_patients_list_name_submit" class="btn btn-primary ms-1 zoom-card">ยืนยัน</button>
                                </div>
                            </div>
                        </form>
                    {{-- Form ดึงรายชื่อคนไข้ที่เสียชีวิต End --}}
                </div>
            </div>
        {{-- All Form Date End --}}

        {{-- แสดงข้อมูลคนไข้ที่เสียชีวิต ตาม วันที่, สถานบริการ, สถานการมีชีวิตหรือเสียชีวิต แบบ Table Start --}}
            <div class="mt-3 card shadow-lg w-100" id="list_of_deceased_patients_list_name_table">
                <div class="row w-100">
        ...
        {{-- แสดงข้อมูลคนไข้ที่เสียชีวิต ตาม วันที่, สถานบริการ, สถานการมีชีวิตหรือเสียชีวิต แบบ Table End --}}
    </main>
@endsection
...
@section('script')
    <script>
        $(document).ready(function() {
            ...
            // เริ่มต้น Hide Start
                $('#list_of_deceased_patients_list_name_table').hide();
                $('.loadingIcon').hide();
            // เริ่มต้น Hide End
            ...
            // Set Text Start
                function setText(request) {
                    if(request == 0) {
                        ...
                        $('#setCount').show();
                        $('#setText').text('');
                        $('#setCount').text('ไม่มีคนไข้ที่เสียชีวิต');
                    } else {
                        $('#setText').show();
                        $('#setCount').show();
                        $('#setText').text('จำนวนคนไข้ที่เสียชีวิตที่แยกตาม รพสต. : ');
            ...
            // Hide Form && Chart && Table Start
                function hideForm() {
                    ...
                    $('#setCount').hide();
                    $('#list_of_deceased_patients_list_name_table').hide();
                    $('#list_of_deceased_patients_list_name_form').hide();
                    $('#select').val('0');
                }
            // Hide Form && Chart && Table End

            // ดึงข้อมูลรายชื่อคนไข้ที่เสียชีวิต ตาม วันที่, เวลา, สถานบริการ, สถานะการมีชีวิตหรือเสียชีวิต Start
                $('#list_of_deceased_patients_list_name_submit').click(function(e) {
                    e.preventDefault();
                    var formData = $('#list_of_deceased_patients_list_name_form').serialize();
                    ...
                    $.ajax({
                        url: '{{ route('getListOfDeceasedPatientsFetchListName') }}',
                        type: 'GET',
                        data: formData,
                        success: function(response) {
                            ...
                            if(response.status === 400) {
                                ...
                                $('#list_of_deceased_patients_list_name_table').show();
                                $('#setText').hide();
                                $('#setCount').hide();
            ...
            // ดึงข้อมูลรายชื่อคนไข้ที่เสียชีวิต ตาม วันที่, เวลา, สถานบริการ, สถานะการมีชีวิตหรือเสียชีวิต End

            // ดึงข้อมูลการเยี่ยมบ้าน รพ.(ครั้ง) แสดงบน Modal Start
                $(document).on('click', '.list-of-deceased-patients-home-visiting-information', function (e) {
                    e.preventDefault();
                    let id = $(this).attr('id');
                    $('#show-list-of-deceased-patients-home-visiting-information').hide();
                    $('#show-list-of-deceaed-patients-home-visiting-information-z718').hide();
                    $('#setText').hide();
                    $('#setCount').hide();
                    ...
                    $.ajax({
                        url: '{{ route('getListOfDeceasedPatientsHomeVisitingInformation') }}',
                        method: 'get',
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        ...
                        success: function(response) {
                            Swal.close();
                            $('#show-list-of-deceased-patients-home-visiting-information').show();
                            $("#show-list-of-deceased-patients-home-visiting-information").html(response);
                        }
                    });
                });
