...
class RPCUController extends Controller
{
    ...
    // หน้าแรกของระบบรวมรายงาน PCU Start
        public function index(Request $request) {
            ...
            if($SidebarSub1MenuId->status_id === 1) {
                ...
                if($accessibility_groupname_model !== null && $accessibility_groupname_model->status_id === 1) {
                    return view('reportes.pcu.index', compact('data', 'sidebar_sub1_menu_id'));
                } else {
                    ...
                    if($accessibility_name_model !== null && $accessibility_name_model->status_id === 1) {
                        ...
                        $request->session()->put('error', 'คุณไม่มีสิทธิ์เข้าใช้งานระบบรวมรายงาน PCU หากต้องการใช้งานกรุณาติดต่อ Admin ของระบบ!');
                        return redirect()->route('dashboard');
                    }
                }
            } else {
                $request->session()->put('error', 'ขณะนี้ระบบรวมรายงาน PCU ไม่ได้เปิดใช้งาน กรุณาแจ้ง Admin หากต้องการใช้งาน!');
                return redirect()->route('dashboard');
            }
        }  
    // หน้าแรกของระบบรวมรายงาน PCU End
}
