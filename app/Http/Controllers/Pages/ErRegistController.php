...
use App\Models\Log\ErRegistLogModel;
use App\Models\Log\ModuleLogModel;
use App\Models\Log\AccessibilityLogModel;

class ErRegistController extends Controller
{
    ...
    // นำปีงบประมาณที่ได้มาจัดหาเดือนที่ถูกต้อง GetYear Start
        private function getYear($year_old, $year_new, $request) {
            ...
            ErRegistLogModel::create($er_regist_log_data);

            return (array) $er_regist_count;
        }
    ...
    // หน้าแรกของ ER Index Start
        public function index(Request $request) {
            ...
            $query_1 = ModuleModel::where('module_name', 'ER');
            ...
            if($erId->status_id === 1) {
                ...
                $query_2 = AccessibilityModel::where('accessibility_name', $data['groupname'])->where('module_id', $erId->id);
                ...
                if($accessibility_groupname_model !== null && $accessibility_groupname_model->status_id === 1) {
                    return view('pages.er_regist', compact('data', 'year', 'fiscal_year'));
                } else {
                    $startTime_3 = microtime(true);

                    $query_3 = AccessibilityModel::where('accessibility_name', $data['name'])->where('module_id', $erId->id);
    ...
    // GetErRegistData Start
        public function getErRegistData(Request $request) {
    ...
    // GetErRegistDailyData Start
        public function getErRegistDailyData(Request $request) {
            try {
                $year = $request->input('year');
                $years = $this->check_year($year);
                ...
                $daily_count = DB::table('er_regist')
                ...
                $er_count = $daily_count->get(); // ดึงข้อมูลออกมา
    ...
    // GetErRegistDailyData End

    // GetErRegistSelectData Start
        public function getErRegistSelectData(Request $request) {
            try {
                ...
                if ($minDate != $maxDate) {
                    ...
                    $daily_count_query = DB::table('er_regist')
                        ->select(DB::raw('DATE(vstdate) as date'), DB::raw('COUNT(*) as count'))
                    ...
                    // สร้างข้อมูลสำหรับบันทึกใน log
                    $er_regist_log_data = [
                        'function' => 'getErRegistSelectData',
                        'username' => $username,
                        'command_sql' => $fullSql, // SQL query ที่มีการแทนค่าจริง
                        'query_time' => $formattedExecutionTime,
                        'operation' => 'SELECT'
