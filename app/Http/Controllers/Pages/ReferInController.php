...
use App\Models\Log\ReferInLogModel;
...
class ReferInController extends Controller
{
    ...
    // นำปีงบประมาณที่ได้มาจัดหาเดือนที่ถูกต้อง GetYear Start
        private function getYear($year_old, $year_new, $request) {
            ...
            // บันทึกข้อมูลลงใน ReferInLogModel
            ReferInLogModel::create($refer_in_log_data);

            // ส่งค่าออกไปพร้อมกับตัวแปรที่เก็บ Array Sart
            return (array) $refer_in_count;
    ...
    // หน้าแรกของ Refer In Index Start
        public function index(Request $request) {
            ...
            $query_1 = ModuleModel::where('module_name', 'Refer In');

            $referInId = $query_1->first();
            ...
            // สร้างข้อมูลสำหรับบันทึกใน log
            $module_log_data = [
                'function' => 'Where module_name = Refer In',
                'username' => $data['loginname'],
            ...
            if($referInId->status_id === 1) {
                $startTime_2 = microtime(true);

                $query_2 = AccessibilityModel::where('accessibility_name', $data['groupname'])->where('module_id', $referInId->id);
                ...
                if($accessibility_groupname_model !== null && $accessibility_groupname_model->status_id === 1) {
                    return view('pages.referin', compact('data', 'year', 'fiscal_year'));
                } else {
                    $startTime_3 = microtime(true);

                    $query_3 = AccessibilityModel::where('accessibility_name', $data['name'])->where('module_id', $referInId->id);
                    ...
                    if($accessibility_name_model !== null && $accessibility_name_model->status_id === 1) {
                        ...
                        $request->session()->put('error', 'คุณไม่มีสิทธิ์เข้าใช้งานระบบ  Refer In หากต้องการใช้งานกรุณาติดต่อ Admin ของระบบ!');
                        return redirect()->route('dashboard');
                    }
                }
            } else {
                $request->session()->put('error', 'ขณะนี้ระบบ  Refer In ไม่ได้เปิดใช้งาน กรุณาแจ้ง Admin หากต้องการใช้งาน!');
                return redirect()->route('dashboard');
            }
        }
    // หน้าแรกของ Refer In Index End

    // GetReferInData Start
    ...
    // GetReferInDailyData Start
        public function getReferInDailyData(Request $request) {
            try {
                ...
                $refer_in_count = $daily_count->get(); // ดึงข้อมูลออกมา
                ...
                $dates = [];
                $counts = [];
                foreach ($refer_in_count as $data) {
                    $dates[] = $data->date;
                    $counts[] = $data->count;
                }
    ...
    // GetReferInDailyData End

    // GetReferInSelectData Start
        public function getReferInSelectData(Request $request) {
            try {
                ...
                if ($minDate != $maxDate) {
                    ...
                    $daily_count_query = DB::table('referin')
                        ->select(DB::raw('DATE(refer_date) as date'), DB::raw('COUNT(*) as count'))
